...
function findLongestWord(sentence) {
  ...
  const wordsLength = wordsArray.map((word) => {
    return word.length;
  });

  const longestWord = Math.max(...wordsLength);
  const indexOfLongestWord = wordsLength.indexOf(longestWord);

  return wordsArray[indexOfLongestWord];
}
...
function invertCase(str) {
  ...
  const reversedChar = splittedStr.map((char) => {
...
function extractNameFromTemplate(value) {
  const fullName = value.slice(7, -1);
  return fullName;
}
...
function encodeToRot13(str) {
  ...
  const encodedStr = splittedStr.map((char) => {
    const charCode = char.charCodeAt(0);
    if (charCode >= 65 && charCode <= 90) {
      const alphabeticIndex = charCode - 65;
      return String.fromCharCode(((alphabeticIndex + 13) % 26) + 65);
    }
    if (charCode >= 97 && charCode <= 122) {
      const alphabeticIndex = charCode - 97;
      return String.fromCharCode(((alphabeticIndex + 13) % 26) + 97);
    }
    return char;
  });
